...
install:
	...
	if rpm -q device-mapper-multipath > /dev/null && [ -d /etc/systemd/system ]; then install -m 644 zfs-multipath-detect.service /etc/systemd/system; systemctl enable zfs-multipath-detect.service; fi
